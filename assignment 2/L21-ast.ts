...
;;         |  ( for <var-decl> <num-exp> <num-exp> <cexp>) / ForExp(var:VarDecl, start:NumExp, end:NumExp, body:CExp)
...
export const parseL21CompoundCExp = (op: Sexp, params: Sexp[]): Result<CExp> =>
    op === "if" ? parseIfExp(params) :
        op === "lambda" ? parseProcExp(first(params), rest(params)) :
            op === "for" ? params.length == 4 && isToken(params[1]) && isToken(params[2]) ? parseForExp(params[0], params[1], params[2], params[3]) :
                makeFailure("Invalid vars for 'for' expression") :
...
const parseForExp = (v: Sexp, n1: Token, n2: Token, params: Sexp[]|Sexp): Result<ForExp> =>
    isString(n1) && isNumericString(n1) && isString(n2) && isNumericString(n2) && +n1<=+n2 && isIdentifier(v) ?
        bind(parseL21CExp(params),
            (cexp: CExp) => makeOk(makeForExp(makeVarDecl(v), makeNumExp(+n1), makeNumExp(+n2), cexp))) :
